...
const { softDeleteMember } = useMembers();
...
const handleConfirmAction = async () => {
  try {
    ...
    await softDeleteMember(props.actionData.userId);
    snackbar.value = {
      show: true,
      message: 'Member has been soft-deleted successfully!',
      color: 'success',
    };
    emit('onSuccess', true);
    ...
    snackbar.value = {
      show: true,
      message: 'Failed to soft-delete the member. Please try again.',
      color: 'error',
    };
  } finally {
...
<template>
  ...
  >
    <BaseDialogCard>
      <BaseDialogTitle>Soft Delete Member</BaseDialogTitle>
      <BaseDialogDescription>
        Are you sure you want to soft delete this member? This action will
        deactivate their account, but it can be restored later if needed.
      </BaseDialogDescription>
      <BaseDialogActions>
        <v-btn
          :disabled="isLoading"
