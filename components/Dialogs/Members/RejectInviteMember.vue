...
const { rejectMemberInvite } = useMembers();
...
const handleConfirmAction = async () => {
  try {
    ...
    await rejectMemberInvite(props.actionData.inviteId);
    snackbar.value = {
      show: true,
    ...
    snackbar.value = {
      show: true,
      message: 'Failed to reject the invite. Please try again.',
      color: 'error',
    };
  } finally {
...
<template>
  ...
  >
    <BaseDialogCard>
      <BaseDialogTitle>Reject Invite</BaseDialogTitle>
      <BaseDialogDescription>
        Are you sure you want to reject the invite for this member? Once
        rejected, the member will no longer have access to the invitation link.
      </BaseDialogDescription>
      <BaseDialogActions>
        <v-btn
          :disabled="isLoading"
        ...
        >
          <div class="capitalize">reject</div>
        </v-btn>
      </BaseDialogActions>
    </BaseDialogCard>
  </v-dialog>
