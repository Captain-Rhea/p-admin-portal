...
const { suspendMember } = useMembers();
...
const handleConfirmAction = async () => {
  try {
    ...
    await suspendMember(props.actionData.userId);
    snackbar.value = {
      show: true,
    ...
    snackbar.value = {
      show: true,
      message: 'Failed to suspend the member. Please try again.',
      color: 'error',
    };
  } finally {
...
<template>
  ...
  >
    <BaseDialogCard>
      <BaseDialogTitle>Suspend Member</BaseDialogTitle>
      <BaseDialogDescription>
        Are you sure you want to suspend this member? They will not be able to
